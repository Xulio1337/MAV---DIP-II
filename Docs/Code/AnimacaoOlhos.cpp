...
void AnimacaoOlhos::desenharOlhos() {
    ...
    _display.fillCircle(PupilaX2, PupilaY2, TamanhoPupila, ST77XX_BLACK);
}
...
void AnimacaoOlhos::expressaoCansado() {
    for (int i = 0; i <= 26; i++) {
        _display.drawLine(0, i, _display.width() - 1, i, ST77XX_BLACK);
    }
}
...
void AnimacaoOlhos::expressaoTriste() {
    ...
    _display.fillTriangle(x1, y1, x2, y2, x3, y3, ST77XX_BLACK);
    ...
    for (int i = 200; i <= 340; i++) {
        for (int j = -espessura; j <= espessura; j++) {
            ...
            _display.drawPixel(x, y, ST77XX_WHITE);
        }
    }

    _display.fillTriangle(x4, y4, x5, y5, x6, y6, ST77XX_BLACK);
}
...
void AnimacaoOlhos::expressaoFeliz() {
    ...
    _display.fillCircle(x1, y1, raio, ST77XX_BLACK);

    int x2 = _display.width() / 1.3;
    int y2 = _display.height() - 1;

    _display.fillCircle(x2, y2, raio, ST77XX_BLACK);
